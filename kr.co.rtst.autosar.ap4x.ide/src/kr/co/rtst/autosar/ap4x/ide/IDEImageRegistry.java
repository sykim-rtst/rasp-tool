...
public class IDEImageRegistry {
	...
	public Image getImage(String imageFilePath){
		...
		if(image == null){
			...
			image = AP4xIDEActivator.getImageDescriptor(imageFilePath).createImage();
	...
	public ImageDescriptor getImageDescriptor(String imageFilePath){
		...
		if(imageDesc == null){
			// 새로운 이미지의 요청이므로 생성하여 등록한다.
			imageDesc = AP4xIDEActivator.getImageDescriptor(imageFilePath);
	...
	public Image getImage(String pluginId, String imageFilePath){
		...
		if(image == null){
			// 새로운 이미지의 요청이므로 생성하여 등록한다.
			image = AP4xIDEActivator.imageDescriptorFromPlugin(pluginId, imageFilePath).createImage();
	...
	public ImageDescriptor getImageDescriptor(String pluginId, String imageFilePath){
		...
		if(imageDesc == null){
			// 새로운 이미지의 요청이므로 생성하여 등록한다.
			imageDesc = AP4xIDEActivator.imageDescriptorFromPlugin(pluginId, imageFilePath);
