...
public class DnsServerIPv4Dialog extends Dialog implements SelectionListener {
	...
	public DnsServerIPv4Dialog(Shell parentShell, Ipv4Configuration ipv4Configuration) {
		super(parentShell);
		this.dnsList = new ArrayList<>();
		for (Inet4Address inet4Address : ipv4Configuration.getDnsServerAddress()) {
			dnsList.add(new AddressModel(inet4Address.getHostAddress()));
		}
	}
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText("IPv4 DNS Configuration");
	}
	...
	private boolean validate() {
		for(TableItem item:tableDnsAddress.getTable().getItems()) {
			if(!ValueUtil.isIPv4Address(item.getText(1))) {
				return false;
			}
		}
		return true;
	...
	public void widgetSelected(SelectionEvent e) {
		if(e.getSource().equals(buttonAddDnsAddress)) {
			dnsList.add(new AddressModel("0.0.0.0"));
			tableDnsAddress.refresh();
			setPageComplete();
