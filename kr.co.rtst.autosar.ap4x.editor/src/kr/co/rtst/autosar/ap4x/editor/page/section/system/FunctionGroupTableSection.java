...
public class FunctionGroupTableSection extends AbstractContentGUISection implements SelectionListener {
	...
	public FunctionGroupTableSection(AbstractAPEditorPage formPage, int style) {
	...
	public void initUIContents() {
		...
		if(input != null) {
			if(input.getType() == null) {
				APModelInitializer.initializeMachine_FunctionGroup(getAPProject(), (Machine)getAPEditorPage().getAPFormEditor().getEditorInputObject(), input);
			}
			tableMachineModeMachine.setInput(input.getType().getModeDeclarations());
	...
	public void widgetSelected(SelectionEvent e) {
		if(e.getSource().equals(buttonAddMachineModeMachine)) {
			final ModeDeclarationGroupPrototype input = getCastedInputDetail();
			if(input != null) {
				doTransactionalOperation(new IAPTransactionalOperation() {
					...
					public GARObject doProcess(GARObject model) throws Exception {
						...
						input.getType().getModeDeclarations().add(md);
						return model;
					}
				});
			...
			if(input != null) {
				doTransactionalOperation(new IAPTransactionalOperation() {
					@Override
					public GARObject doProcess(GARObject model) throws Exception {
						input.getType().getModeDeclarations().removeAll(tableMachineModeMachine.getStructuredSelection().toList());
						return model;
					}
				});
		...
		tableMachineModeMachine.setCellModifier(new ICellModifier() {
				...
				doTransactionalOperation(new IAPTransactionalOperation() {
					...
					public GARObject doProcess(GARObject arg) throws Exception {
						if(TABLE_COLUMN[1].equals(property)) {
							...
							if( ((Integer)value) == 0 ) {
								getCastedInputDetail().getType().setInitialMode(model);
							}else {
								if(model.equals(getCastedInputDetail().getType().getInitialMode())){
									getCastedInputDetail().getType().setInitialMode(null);
								}
							}
						}
						return model;
			...
			public Object getValue(Object element, String property) {
				...
				if(TABLE_COLUMN[1].equals(property)) {
					...
					return model.equals(getCastedInputDetail().getType().getInitialMode())?0:1;
				}
				return null;
			}
...
//				case 0: return "";
				...
				case 3: return model.equals(getCastedInputDetail().getType().getInitialMode())?APSectionUIFactory.COMBO_BOOLEAN_VALUE[APSectionUIFactory.COMBO_BOOLEAN_VALUE_INDEX_TRUE]:APSectionUIFactory.COMBO_BOOLEAN_VALUE[APSectionUIFactory.COMBO_BOOLEAN_VALUE_INDEX_FALSE];
				}
			}
			return "";
		}
