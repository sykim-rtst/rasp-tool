...
public class GARObjectMultiSelectionDialog extends Dialog {
	
	private String selectElementName;
	private FormToolkit toolkit;
	private GARObjectMultiFilteredList garObjectList;
	
	private List<GARObject> objectList;
	private List<GARObject> selectedObjects;
	 ...
	 * @param title
	 */
	public GARObjectMultiSelectionDialog(Shell parentShell, FormToolkit toolkit, List<GARObject> objectList, String selectElementName) {
		super(parentShell);
...
//		setTitle(String.format(IDEText.DIALOG_MESSAGE_NEW_ELEMENT, createElementName));
	...
	protected Control createDialogArea(Composite parent) {
		...
		garObjectList = new GARObjectMultiFilteredList(treeComp, toolkit);
	...
	protected void okPressed() {
		...
		if(!garObjectList.getTreeViewer().getStructuredSelection().isEmpty()) {
			selectedObjects.addAll(garObjectList.getTreeViewer().getStructuredSelection().toList());
		}
		super.okPressed();
	}
	
	public List<GARObject> getSelectedObjects() {
	...
	protected void setPageComplete() {
		...
		if(button != null && !button.isDisposed()){
			if(garObjectList.getTreeViewer().getStructuredSelection().size() > 0) {
	...
	private class GARPackageLabelProvider extends LabelProvider{
		...
		public String getText(Object element) {
			...
			if(result == null || result.trim().length()==0) {
				result =  apModelLabelProvider.getText(element);
			}
			if(result == null) {
				result = "";
			}
